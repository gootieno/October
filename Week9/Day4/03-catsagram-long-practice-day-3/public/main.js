...
export const createMainContent = async () => {
  ...
  let kittenImg = localStorage.getItem("catImage");
  console.log("kitten image in main content container ", kittenImg);
  if (kittenImg) {
    img.src = kittenImg;
  } else {
...
const fetchImage = async () => {
  ...
  try {
    ...
    localStorage.setItem("catImage", kittenImgUrl);
    return kittenImgUrl;
...
const createNewKittenBtn = () => {
  ...
  newKittenBtn.addEventListener("click", async () => {
    ...
    const kittenImage = document.getElementById("cat-image");
    kittenImage.setAttribute("src", kittenImageUrl);
  });
  return newKittenBtn;
};
